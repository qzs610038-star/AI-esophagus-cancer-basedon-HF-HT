...
- 内部验证按预定规则选择`spatial_joint`：逐通路平均 PCC / 整体展平 PCC = `0.6734 / 0.8090`，zMSE=`0.3552`。
- XZY外部：逐通路平均 PCC / 整体展平 PCC = `0.5582 / 0.6720`，zMSE=`0.6418`；相对来源step0分别`+0.0157 / +0.0088 / -0.0221`。
- 固定β=1仅为诊断：双PCC=`0.5779 / 0.6821`，zMSE=`0.6447`，未参与选模。
- 登记状态：`registered`；证据状态：`accepted`。仅支持单种子、单外部患者范围，不证明广泛跨患者泛化或全面优于固定平滑。
