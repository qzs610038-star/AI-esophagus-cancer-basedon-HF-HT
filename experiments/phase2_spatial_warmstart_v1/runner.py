...
def main():
    ...
    try:
        ...
        entry_args = list(package["args"])
        if args.mode is not None:
            entry_args = [args.mode, "--arm", args.arm]
            if args.mode == "resume":
                if args.resume_checkpoint is None:
                    raise ValueError("--resume-checkpoint is required for resume mode")
                entry_args.extend(["--checkpoint", str(args.resume_checkpoint.resolve())])
        command = [
            sys.executable, "-u", str(package_dir / package["entrypoint"]), *entry_args,
