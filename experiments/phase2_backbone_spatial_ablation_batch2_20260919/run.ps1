param(
    ...
    [ValidateSet('check-environment','check-inputs','prepare-features','train-spatial','external-eval','analyze-local')]
    [string]$Action = 'check-inputs',
    [string]$BatchDir,
    [string]$Device = 'cuda',
    [ValidateSet('hoptimus0','hoptimus1','phikonv2')]
    [string[]]$Models,
...
try {
    ...
    Write-JsonFile $recordPath $record
    $runnerArgs = @('-X', 'utf8', '-u', (Join-Path $PSScriptRoot 'runner.py'), '--run-dir', $runDir, '--weights-dir', $weightDir, '--action', $Action, '--device', $Device)
    if ($Models) { $runnerArgs += @('--models') + $Models }
