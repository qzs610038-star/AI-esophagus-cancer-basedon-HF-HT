...
团队已选定本包 `spatial` 臂作为方案 A（经典中间残差 / 经典空间残差）。同目录三份队友说明请按读者分开用，不要混用：

| 文件 | 读者 | 用途 |
|---|---|---|
| `给phase3队友的交接说明_方案A经典中间残差.md` | phase3 / Phase 3 | 权重路径 + 如何接入 30 维 z-score 疗效管线 |
| `给队友的任务说明.md` | Phase 2 消融（任务 2 / 3） | 同一份权重接到基因→通路、隔点训练 |
| `队友同步说明.md` | 同上消融 | 改哪些代码、加载哪些 state 键 |

默认只取种子 42 的 `formal_best.pt`。本次交接不要求重新训练本包四臂，也不要求跑 43、44。
