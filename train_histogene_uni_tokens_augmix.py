"""
HisToGene-UNI Token序列训练脚本 — AugMix + MixUp 增强版
========================================================
基于 train_histogene_uni_tokens.py，新增：
  1. AugMix数据集：训练时随机采样 H&E 增强版 token 特征
  2. Token-level MixUp：batch 内对 token 特征和标签做线性插值
  3. 全部增强参数通过命令行控制，默认可回退为基线行为
...
约束：
  ...
  - 不修改 model_uni_tokens.py / dataset_uni_tokens.py / train_histogene_uni_tokens.py
  - 模型类复用 model_uni_tokens.HisToGeneUNITokens
"""
...
from dataset_uni_tokens_augmix import HisToGeneUNITokensAugMixDataset
...
def mixup_batch(tokens, targets, alpha=1.0):
    """Batch级别的 Manifold MixUp（token层）

    对 batch 内的 token 特征和标签进行线性插值混合，
    坐标 (pos_x, pos_y) 保持不变。

    Args:
        tokens: (B, num_tokens, feature_dim) token特征
        targets: (B, n_targets) 标签
        alpha: Beta分布参数，alpha>0 时启用MixUp，alpha<=0 返回原数据

    Returns:
        tokens_mix: 混合后的token特征
        targets_mix: 混合后的标签
    """
    if alpha <= 0:
    ...
    tokens_mix = lam * tokens + (1 - lam) * tokens[index]
    targets_mix = lam * targets + (1 - lam) * targets[index]
    return tokens_mix, targets_mix
...
def generate_model_params_txt(args, n_params, history_df, output_path,
    ...
    lines = []
    lines.append("=" * 50)
    lines.append("HisToGene-UNI Token序列训练参数（方案B-AugMix）")
    lines.append("=" * 50)
    ...
    lines.append("=== 模型参数 ===")
    lines.append(f"方案: B-AugMix (UNI Token序列 + 数据增强)")
    ...
    lines.append("=== 增强参数 ===")
    lines.append(f"use_augmented_tokens: {args.use_augmented_tokens}")
    lines.append(f"n_augments: {args.n_augments}")
    lines.append(f"aug_sample_prob: {args.aug_sample_prob}")
    lines.append(f"mixup_alpha: {args.mixup_alpha}")
    lines.append(f"mixup_prob: {args.mixup_prob}")
    lines.append("")
    ...
    lines.append(f"方案B-AugMix Best Val PCC: {best_val_pcc:.4f}")
...
def train_one_epoch(model, loader, optimizer, criterion, device,
                    ...
                    mixup_alpha=0.0, mixup_prob=0.5):
    """单epoch训练，支持MixUp和标签噪声

    Args:
        mixup_alpha: Beta分布参数，>0启用MixUp，0或负数禁用
        mixup_prob: 每个batch应用MixUp的概率
    """
    model.train()
    total_loss = 0.0
    ...
    for tokens, pos_x, pos_y, targets in loader:
        ...
        # MixUp（仅在训练时，以mixup_prob概率应用）
        # 注意：坐标(pos_x, pos_y)不参与混合，保持原样本坐标
        if mixup_alpha > 0 and random.random() < mixup_prob:
            tokens, targets = mixup_batch(tokens, targets, alpha=mixup_alpha)

        # 标签噪声（与MixUp兼容：在MixUp混合后再加噪声）
        if label_noise > 0:
...
def build_argparser():
    p = argparse.ArgumentParser(
        description="HisToGene-UNI Token序列训练（方案B-AugMix + MixUp）"
    )
    ...
    # ─── 增强相关 ───
    p.add_argument("--use_augmented_tokens", action="store_true",
                   help="使用H&E增强版特征目录")
    p.add_argument("--augmented_tokens_dir", type=str,
                   default="uni2h_cache_tokens_aug",
                   help="增强特征缓存根目录")
    p.add_argument("--n_augments", type=int, default=3,
                   help="增强变体数量")
    p.add_argument("--aug_sample_prob", type=float, default=0.5,
                   help="训练时选择增强版本的概率")

    # ─── MixUp相关 ───
    p.add_argument("--mixup_alpha", type=float, default=1.0,
                   help="MixUp Beta分布参数 (0=禁用)")
...
def main():
    ...
    if args.dataset_name is None:
        if args.patient:
            args.dataset_name = f"{args.patient}_UNI_tokens_AugMix"
        else:
            if args.fold == 1:
    ...
    best_ckpt = ckpt_dir / "best_histogene_uni_tokens_augmix.pth"
    resume_ckpt = ckpt_dir / "resume_histogene_uni_tokens_augmix.pth"

    # ── 打印信息 ──────────────────────────────────────────────────────────
    print("=" * 70)
    print("HisToGene-UNI Token序列训练（方案B-AugMix + MixUp）")
    if args.patient:
    ...
    print(f"  AugTokens: enabled={args.use_augmented_tokens}, n_aug={args.n_augments}, prob={args.aug_sample_prob}")
    print("=" * 70)
    ...
    if args.patient:
        ...
        aug_train_dir = pc['token_aug_train'] if args.use_augmented_tokens else None

        train_dataset = HisToGeneUNITokensAugMixDataset(
            patches_dir=pc['train_patches'],
            feature_cache_dir=pc['token_cache_train'],
            labels_csv=pc['labels_csv'],
            augmented_cache_dir=aug_train_dir,
        ...
        val_dataset = HisToGeneUNITokensAugMixDataset(
            patches_dir=pc['val_patches'],
            feature_cache_dir=pc['token_cache_val'],
            labels_csv=pc['labels_csv'],
            augmented_cache_dir=None,  # 验证集不用增强
            is_train=False,
        ...
        for pname in train_patient_names:
            ...
            for split, patches_key, cache_key, aug_key in [
                ...
                if args.use_augmented_tokens:
                    config_item['augmented_cache_dir'] = pc[aug_key]
                train_patient_configs.append(config_item)

        train_dataset, coord_stats_dict, target_cols = \
            HisToGeneUNITokensAugMixDataset.from_multiple_patients(
    ...
    task_name = f"HisToGene-UNI-Tokens-AugMix_{args.dataset_name}"

    print("\n" + "=" * 90)
    print(f"开始训练 方案B-AugMix | Epochs={args.num_epochs} | BS={args.batch_size} | LR={args.lr}")
    print(f"  训练集: {len(train_dataset)} 样本 | {val_label}集: {len(val_dataset)} 样本")
    print(f"  MixUp: alpha={args.mixup_alpha}, prob={args.mixup_prob}")
    aug_status = "启用" if args.use_augmented_tokens else "未启用"
    print(f"  AugTokens: {aug_status}")
    ...
    print(f"  方案A基线 Val PCC = 0.577, 方案B-AugMix Best Val PCC = {best_pcc:.4f}")
    ...
    try:
        ...
        # 生成完整可视化报告
        model_name_with_dataset = f"HisToGene-UNI-Tokens-AugMix_{args.dataset_name}"
        generate_full_report(
            ...
            params={
                ...
                "use_augmented_tokens": args.use_augmented_tokens,
                "n_augments": args.n_augments,
                "aug_sample_prob": args.aug_sample_prob,
            }
        )
