...
> **已失效，禁止执行（2026-08-01 用户批准）**：正式 r003 外部门失败且证据已被拒绝；本提示词只保留实验历史。`CalibratedMPP2` 不进入 Phase 3 或部署，不得以本文件重新创建任务、重试或依据 XZY 调参。当前唯一可进入 Phase 3 增量验证的 MPP2 候选是 accepted 修复版冻结基线的原始输出。
