# PFMval 本地代码 Git 治理与 W### 目录化执行方案（待审核）
...
- [ ] 非代码治理先形成独立基线提交。
- [ ] 102 个时间候选全部获得唯一分类。
- [ ] 三旧目录不再被 active import 直接引用。
- [ ] 旧代码目录只包含批准的 MPP1-5 前代码。
- [ ] 没有数据、checkpoint、缓存、结果或文献混入历史代码目录。
- [ ] 新 W### 目录包含代码、配置、测试、日志索引和 closeout。
- [ ] 原始日志和训练产物仍位于工作树外。
- [ ] 可复用代码通过独立审核晋升 main。
- [ ] 最简相关测试通过。
- [ ] 严格门禁未新增由本次治理造成的 FAIL。
- [ ] 未执行任何未经明确批准的远端推送。

## 9. 当前门禁状态

完成约束文件窄范围更新后，使用 `D:\conda_envs\pfmval_py310\python.exe` 重新运行严格门禁得到：
...
这些 FAIL 必须在真正迁移前处理。本方案文件存在不代表迁移已获批准或已经执行。

## 10. 待用户审核决定

1. 是否批准以 2026-06-30 23:59:59 为旧代码时间上限？
2. 是否批准 B 批优先处理三旧目录及其直接调用方？
3. `historical_code/` 是否确认保持本地只读、Git 忽略，仅以迁移前 Git 提交作为恢复来源？
4. 是否接受 W### “整包先合入、公共代码再晋升、完整包最终只留 Git 历史和 closeout 索引”的收口方式？
5. 是否批准按第 9 节申报的范围，刷新四个 active 文本文件的既有 SHA-256 登记指纹？
