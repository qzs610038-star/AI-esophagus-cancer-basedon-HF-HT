"""
EGNv2 训练脚本 - PFMval 项目
完整训练流程：数据加载、特征提取、图构建、模型训练、验证、早停、保存
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train EGNv2 on ssGSEA pathway scores.")
    ...
    p.add_argument("--cache_dir", type=str, default=None,
                   help="特征/图缓存目录，默认为 egnv2/cache/{dataset_name}")
...
def main():
    ...
    if args.multi_patient:
        ...
        # 单患者模式
        train_dataset = EGNv2Dataset(
            patches_dir=args.train_patches_dir,
            labels_csv=args.labels_csv,
            transform=train_transform,
        )

        val_dataset = EGNv2Dataset(
    ...
    print("\n" + "=" * 90)
    print(f"开始训练 EGNv2 | Epochs={args.num_epochs} | LR={args.lr} | "
    ...
    try:
        for epoch in range(start_epoch, args.num_epochs + 1):
            ...
            if check_pause_signal(_PROJECT_ROOT):
                ...
                notify_training_complete("EGNv2", epoch, best_epoch, best_pcc, "paused")
                clear_pause_signal(_PROJECT_ROOT)
                return

    except Exception as e:
        notify_training_error("EGNv2", current_epoch, str(e))
        raise

    # 训练完成通知
    status = "early_stop" if early_stopped else "completed"
    notify_training_complete("EGNv2", current_epoch, best_epoch, best_pcc, status)
