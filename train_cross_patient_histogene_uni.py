...
约束：
  ...
  - 复用 histogene 核心组件（HisToGeneUNIDataset, HisToGeneUNI, compute_metrics）
  - 可视化输出与现有训练脚本一致
"""
...
from histogene.train_uni import ensure_features_cached
...
def generate_model_params_txt(args, n_params, history_df, output_path,
    ...
    lines = []
    lines.append("=" * 50)
    lines.append("HisToGene-UNI 跨患者泛化训练参数")
    lines.append("=" * 50)
...
def build_argparser():
    ...
    p.add_argument("--label_noise",   type=float, default=0.0,
                   help="训练标签高斯噪声标准差（回归版label smoothing）")
...
def main():
    ...
    for label, path in required_cache_dirs:
        if not os.path.isdir(path):
            print(f"[ERROR] {label} 不存在: {path}")
            print(f"  请先运行 extract_uni_features_3st.py 提取UNI2-h特征缓存")
            sys.exit(1)
        n_cache = len([f for f in os.listdir(path) if f.lower().endswith('.pt')])
        print(f"  {label}: {n_cache} cached features")
    ...
    test_dataset, test_coord_stats_dict, _ = HisToGeneUNIDataset.from_multiple_patients(
    ...
    try:
        ...
        all_labels = []
        with torch.no_grad():
            for features, pos_x, pos_y, targets in test_loader:
                features = features.to(device, non_blocking=True)
                pos_x = pos_x.to(device, non_blocking=True)
                pos_y = pos_y.to(device, non_blocking=True)
                preds = model(features, pos_x, pos_y)
