...
Use `python deploy/pfmval_ops.py agent start-check` when its diagnostics help; `--strict`, Registry alignment, W### identity, transport choice, approvals stored as files, and hashes are not universal prerequisites. Treat them as warnings unless the claim specifically depends on them. Hashing is optional and should be used only when the user requests identity verification of a named critical artifact.
