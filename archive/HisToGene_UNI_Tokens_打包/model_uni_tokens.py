...
class LightweightTokenEncoder(nn.Module):
    ...
    def forward(self, tokens):
        """
        Args:
            tokens: [B, num_tokens, feature_dim]
        Returns:
            encoded: [B, embed_dim]
        """
        ...
        # TokenDrop: 训练时随机丢弃 token，迫使模型不依赖特定空间位置
        if self.training and self.token_drop_rate > 0:
            keep_prob = 1.0 - self.token_drop_rate
            mask = torch.rand(x.size(0), x.size(1), 1, device=x.device) < keep_prob
            x = x * mask.float() / keep_prob
