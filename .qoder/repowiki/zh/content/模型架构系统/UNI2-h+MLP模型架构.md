...
- DenseNet风格回归头：基于DenseNet-121结构设计的增强回归模型，包含密集连接块和过渡层。
...
### DenseNet风格回归头（增强版本）
- 结构设计：基于DenseNet-121架构，包含4个密集连接块，层数分别为[6, 12, 24, 16]
- 密集连接：每层输出与前面所有层特征拼接，形成密集连接结构
- 过渡层：在密集块之间使用线性层和ReLU激活，控制特征维度
...
- 泛化能力
  ...
  - UNI2-h+DenseNet：结合密集连接结构，进一步提升特征融合效果。
...
- 指标丰富度
  ...
  - UNI2-h+DenseNet：额外支持MAPE指标，提供更全面的性能评估。
  - HisToGene：支持标准回归指标。
...
## 结论
UNI2-h+MLP两阶段架构通过"特征冻结 + 轻量回归"的设计，在保证预测性能的同时大幅提升了训练与推理效率。特征缓存系统有效避免重复计算，使模型在大规模数据上具备良好的可扩展性。增强版本的DenseNet回归头进一步提升了模型表达能力，提供了更全面的性能评估指标。对于需要显式空间信息的任务，可结合HisToGene模型；对于追求高效率与强泛化的任务，UNI2-h+MLP是更优选择。
...
- 指标表现：UNI2-h+DenseNet在多个指标上表现更佳，特别是MAPE指标。
