"""
train_online_cls.py — UNI2-H 在线 CLS 模式训练脚本
====================================================

支持渐进式三阶段训练：
  Stage 1 (lora):     全 24 层注入 LoRA (qkv+proj)，backbone 其余冻结
  Stage 2 (stage2):   移除末 2 层 LoRA + 解冻末 2 层全部参数
  Stage 3 (stage3):   移除末 4 层 LoRA + 解冻末 4 层全部参数

以及 frozen 基线模式和完整的 cross-patient / 3-fold CV。

用法:
  # Frozen 基线
  python train_online_cls.py --mode frozen --fold 1

  # Stage 1 LoRA (rank=8)
  python train_online_cls.py --mode lora --lora_rank 8 --fold 1

  # Stage 2 渐进 (从 Stage 1 checkpoint 加载)
  python train_online_cls.py --mode stage2 --lora_rank 8 --resume <stage1_ckpt> --fold 1

  # Stage 3 渐进
  python train_online_cls.py --mode stage3 --lora_rank 8 --resume <stage2_ckpt> --fold 1
"""
...
from model_online_cls import OnlineCLSModel
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--batch_size", type=int, default=8,
                   help="批大小 (服务器默认8, 本地8GB用1)")
    p.add_argument("--grad_accum_steps", type=int, default=1,
                   help="梯度累积步数 (batch_size=1时用4)")
...
def main():
    ...
    if args.dataset_name is None:
        if args.cross_patient:
            args.dataset_name = f"online_cls_cross_fold{args.fold}"
        elif args.patient:
    ...
    task_name = f"OnlineCLS-{args.mode}-r{args.lora_rank}_{args.dataset_name}"
