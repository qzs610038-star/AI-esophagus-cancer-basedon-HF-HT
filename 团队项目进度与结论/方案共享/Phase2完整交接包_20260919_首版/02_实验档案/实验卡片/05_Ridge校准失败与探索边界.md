...
- **解释边界：** 包内旧快照的 `next_action` 是当时建议；9 月 19 日现行登记已明确改为 `closed_failed_rejected_no_automatic_rerun_or_deployment`（失败关闭、不自动重跑或部署），原建议另存为历史字段，当前字段见[治理刷新摘录](../../06_证据摘录/治理刷新事实摘录_20260919.json)。本卡只记录已有结果，不能据此称线性校准无效或已获准重跑。
